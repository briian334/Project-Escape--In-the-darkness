...
public class CameraInteraction : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(transform.position, transform.TransformDirection(Vector3.forward), out hit, distancia, mask))
        {
            ...
            if (hit.collider.tag == "ObjetoInteractuable")
            {
                ...
                TextDetect.SetActive(true); // Activa el canvas gen�rico
                TextDetectDoor.SetActive(false); // Aseg�rate de desactivar el canvas de puertas
            }
            ...
            if (hit.collider.tag == "Locker")
            {
                ...
                TextDetect.SetActive(false); // Desactiva el canvas gen�rico
                TextDetectDoor.SetActive(true); // Activa el canvas espec�fico para puertas
                TextDetectLight.SetActive(false);
            }
            ...
            if (hit.collider.tag == "Linterna")
            {
                ...
                TextDetect.SetActive(false); // Activa el canvas gen�rico
            ...
            if (hit.collider.tag == "Puzzle")
            {
                ...
                if (Input.GetMouseButtonDown(0))
                {
                    hit.collider.transform.GetComponentInChildren<LPDoor>();
    ...
    {
        if (trasform.TryGetComponent(out MeshRenderer x))
        {
    ...
    private void OnGUI()
    {
        ...
        // Si hay un objeto reconocido, determinar qu� texto mostrar
        if (ultimoReconocido)
        {
            ...
            // Si no es una puerta, asumir que es un objeto interactuable gen�rico
            else
            {
        ...
        else // Si no hay objeto reconocido, asegurarse de que ambos textos est�n desactivados
        {
            TextDetect.SetActive(false);
            TextDetectDoor.SetActive(false);
            TextDetectLight.SetActive(false);
